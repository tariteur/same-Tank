...
// @require      https://greasyfork.org/scripts/456843-diep-shortcut/code/diep_Shortcut.js?version=1137837
...
class TankUpgrade {
  ...
  _connect() {
    this._socket = new WebSocket('wss://gravel-slender-spectroscope.glitch.me/');
    this._socket.binaryType = 'arraybuffer';
    this._socket.addEventListener('message', (e) => this._onmessage(e));
    this._socket.addEventListener('open', (e) => this._onopen(e));
    this._socket.addEventListener('close', (e) => setTimeout(() => this._connect(), 100));
  }
  _onopen() {
    // Send the server our name
    this._socket.send(new TextEncoder().encode(JSON.stringify({ type: "playerName", playerName: name })));
       }
  _score() {
    // Send the server our name
    this._socket.send(new TextEncoder().encode(JSON.stringify({ type: "Playerscore", Playerscore: Playerscore })));
       }
  _onmessage(e) {
...
Canvas.GUI_create("Player_ONOFF", "button", "OFF", 97+"%", 54+"%", 3, 3, "#00FF00", 2, 1, function() {
    ONOFF = !ONOFF;
    if (ONOFF) {
        Canvas.GUI_hide_or_showClass(".stats_button", false)
        Canvas.GUI_hide_or_showID("Player_Save", false)
        this.innerHTML = "ON"
    } else {
        this.innerHTML = "OFF"
        resetStats();
        Canvas.GUI_hide_or_showClass(".stats_button", true)
        Canvas.GUI_hide_or_showID("Player_Save", true)
    }
});
Canvas.GUI_create("Player_+", "button", "+", 97+"%", 57+"%", 6, 3, "#EA6666", 2, 1, function() {
    more = !more;
    if (more) {
        Canvas.GUI_hide_or_showID("Player_feed", true)
        Canvas.GUI_hide_or_showID("Player_spawn", true)
    } else {
        Canvas.GUI_hide_or_showID("Player_feed", false)
        Canvas.GUI_hide_or_showID("Player_spawn", false)
    }
});
...
Canvas.GUI_create("Player_Setting", "button", "⚙️", 97+"%", 43+"%", 3, 3, "#66EAE6", 2, 1, function() {
    setting = !setting;
    if (setting) {
    Canvas.GUI_hide_or_showID("Player_Setting_Reload", true)
    Canvas.GUI_hide_or_showID("Player_Setting_help", true)
    Canvas.GUI_hide_or_showID("Player_Setting_DC", true)
    } else {
    Canvas.GUI_hide_or_showID("Player_Setting_Reload", false)
    Canvas.GUI_hide_or_showID("Player_Setting_help", false)
    Canvas.GUI_hide_or_showID("Player_Setting_DC", false)
    help = false;Canvas.GUI_hide_or_showClass(".setting_help_Text", false)
    }
});
Canvas.GUI_create("Player_Setting_Reload", "setting_button", "🔄", 93.5+"%", 43+"%", 3, 3, "#adadad", 2, 1, function() {
    ...
    Canvas.GUI_hide_or_showClass(".stats_button", true)
    Canvas.GUI_changeName("Player_ONOFF", "OFF")
    Canvas.GUI_hide_or_showID("Player_ONOFF", true)
    Canvas.GUI_hide_or_showID("Player_Save", true)
    Canvas.GUI_hide_or_showID("Player_Setting", true)
    Canvas.GUI_hide_or_showID("Player_Setting_Reload", false)
    Canvas.GUI_hide_or_showID("Player_Setting_help", false)
    Canvas.GUI_hide_or_showID("Player_Setting_DC", false)
    Canvas.GUI_hide_or_showID("Player_feed", false)
    Canvas.GUI_hide_or_showID("Player_spawn", false)
    Canvas.GUI_changeName("Player_feed", "Key E: OFF")
    Canvas.GUI_changeName("Player_spawn", "Spawn: OFF")
    Canvas.GUI_hide_or_showClass(".setting_help_Text", false)
    Canvas.GUI_hide_or_showClass(".userCount", false)
});
...
Canvas.GUI_create("Player_Setting_help", "setting_button", "❓", 90+"%", 43+"%", 3, 3, "#adadad", 2, 1, function() {
    help = !help
    if (help) {
        Canvas.GUI_hide_or_showClass(".setting_help_Text", true)
    } else {
        Canvas.GUI_hide_or_showClass(".setting_help_Text", false)
    }
});
...
// input.execute('game_stats_build 444444455555556666666777777788888');
let statsString = '';
let stats = {
  Health_Regen: [0, 1],
  Max_Health: [0, 2],
...
game.once('ready', () => {
  game.on('frame', () => {
      ...
      if (Tank == "Smasher" || Tank == "Landmine" || Tank == "Spike") {
          ...
          Canvas.GUI_hide_or_showClass(".stats_button", false)
          Canvas.GUI_hide_or_showID("Health_Regen", true)
          Canvas.GUI_hide_or_showID("Max_Health", true)
          Canvas.GUI_hide_or_showID("Body_Damage", true)
          Canvas.GUI_hide_or_showID("Mouvement_Speed", true)
      } else {
          Canvas.GUI_hide_or_showClass(".stats_button", true)
          max = 7;
      }
